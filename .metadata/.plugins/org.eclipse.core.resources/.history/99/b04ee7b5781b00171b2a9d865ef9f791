		...
		try {
			...
			while (sc.hasNextLine()) { 
				...
				if (input[0].equals("Request")) {
					...
					while (numManual != 0) {
						for (int order = 0; order < depotOrder; order++) {
							CamperVan bookedVan = depotMap.get(order).getManualVan(depotOrder);	// Grabs van object
							System.out.println(bookedVan);
							vanList.add(bookedVan);												// Adds van to curr vanList
						}
						numManual--;
					...
					Booking request = new Booking(vanList, bookingID, startBooking, endBooking, numAuto, numManual);	// create new booking
